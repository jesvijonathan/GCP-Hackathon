<template>
  <div class="dash-data-controls" :class="{ open, intro:introVisible }" ref="rootEl">
    <div class="ddc-header" @click="toggle" title="Click to collapse/expand">
      <strong style="flex:1;">Merchant Data / Time</strong>
      <button class="toggle-btn" type="button" @click.stop="toggle">{{ open ? '×' : '⋯' }}</button>
    </div>
    <div v-if="open" class="ddc-body">
      ...
      <div v-if="windowMode" class="section grid2 align-top">
        <div class="field-group">
          <label class="lbl" title="Examples: 6h,12h,1d,7d">Window</label>
          <input v-model.trim="windowVal" class="inp" placeholder="7d" @keydown.enter.prevent="enterFetch" />
        </div>
        <div class="field-group">
          <label class="lbl">Aggregate</label>
          <select v-model="unit" class="inp" @keydown.enter.prevent="enterFetch">
      ...
      <div v-else class="section">
        <div class="grid2">
          <div>
            ...
            <input v-model.trim="sinceStr" class="inp" placeholder="2025-01-01T00:00:00Z" @keydown.enter.prevent="enterFetch" />
          </div>
          <div>
            <label class="lbl">Until (ISO/epoch)</label>
            <input v-model.trim="untilStr" class="inp" placeholder="Optional" @keydown.enter.prevent="enterFetch" />
          </div>
        </div>
      ...
      <div class="section grid2">
        <div>
          ...
          <input type="number" min="1" v-model.number="limit" class="inp" />
        </div>
        <div>
          <label class="lbl">Order</label>
          <select v-model="order" class="inp">
            <option value="desc">Newest First</option>
            <option value="asc">Oldest First</option>
          </select>
        </div>
      </div>
      <!-- Allow Future (moved auto refresh lower) -->
      ...
      <div class="section sim-block">
        ...
        <div v-else class="section" style="margin-top:4px;">
          ...
          <input v-model.trim="nowIso" class="inp" placeholder="Leave blank for real now" @keydown.enter.prevent="enterFetch" />
        </div>
      </div>
      <!-- Auto Refresh relocated below simulation block -->
      <div class="section auto-refresh-wrap">
        <div class="row aic gap6 no-wrap auto-refresh-row">
          ...
          <div v-if="autoRefresh" class="refresh-inline">
            <input type="number" min="2" v-model.number="refreshSec" class="inp inp-sm refresh-input" @keydown.enter.prevent="enterFetch" placeholder="10" />
            <span class="ri-s">s</span>
          </div>
        </div>
        <transition name="fade">
          <div v-if="autoRefresh" class="refresh-meta">
            <div class="refresh-countdown" :title="'Next refresh in ' + nextRefreshIn + 's'">
              Next: <strong>{{ nextRefreshIn }}</strong>s
            </div>
            <div class="refresh-counter" :title="'Auto refresh count'">
              Count: <strong>{{ refreshCount }}</strong>
            </div>
          </div>
        </transition>
      </div>
      <div class="section row gap6">
        <button type="button" class="btn primary" :disabled="loading" @click="emitFetch">{{ loading ? 'Fetching...' : 'Fetch' }}</button>
        <button type="button" class="btn alt" @click="toggleMode">{{ windowMode ? 'Use Since/Until':'Use Window' }}</button>
      </div>
    </div>
  </div>
</template>
...
// Re-use same simulation logic key so merchant + dashboard share time when desired
...
function useSim(){
  ...
  const simNow = computed(()=>{
    ...
    const elapsed = simClock.value - wallStartMs; const adv = simRunning.value ? elapsed * Math.max(0,accel.value) : 0;
...
export default {
  ...
  // Intro visibility keeps opacity at 1 for first 2s even when collapsed
  const introVisible = ref(true);
    const rootEl = ref(null);
  const windowMode = ref(true);
    const windowVal = ref('7d');
    const sinceStr = ref(''); const untilStr = ref('');
    const unit = ref('day');
    const limit = ref(5000); const order = ref('desc');
  ...
  const nextRefreshIn = ref(0); // seconds remaining until next auto fetch
  const refreshCount = ref(0); // number of auto refresh fetches (including initial auto-start fetch)
  const STATE_KEY = 'merchantDataCtrlState_v1';
  let lastAppliedTs = 0;
    ...
    function enableWindowMode(){ windowMode.value=true; if(!windowVal.value) windowVal.value='7d'; }
    function enableRangeMode(){ windowMode.value=false; if(!sinceStr.value) sinceStr.value=new Date(Date.now()-24*3600*1000).toISOString(); }
    function toggleMode(){ windowMode.value = !windowMode.value; if(windowMode.value){ enableWindowMode(); } else { enableRangeMode(); } }
    function payload(){ return { window: windowMode.value? windowVal.value:'', since: windowMode.value? '': sinceStr.value, until: windowMode.value? '': untilStr.value, unit: unit.value, limit: limit.value, order: order.value, allowFuture: allowFuture.value, now: simEnabled.value? simNow.value : nowIso.value }; }
    function emitFetch(force=false, origin='manual'){
      if(!force && props.loading) return;
      const pl = payload();
      try { console.log('[MDC] emit fetch', pl); } catch {}
      emit('fetch', pl);
      // If a manual fetch occurs while auto refresh is active, reset countdown so user sees full interval again
      if(autoRefresh.value && origin==='manual'){
        nextRefreshIn.value = refreshSec.value;
      }
    }
    function enterFetch(){ emitFetch(); }
    function scheduleAuto(){
      if(refreshTimer){ clearInterval(refreshTimer); refreshTimer=null; }
      if(countdownTimer){ clearInterval(countdownTimer); countdownTimer=null; }
      if(autoRefresh.value){
        const intervalMs = Math.max(2, refreshSec.value)*1000;
        nextRefreshIn.value = Math.max(2, refreshSec.value);
        refreshTimer = setInterval(()=> {
          emitFetch(false,'auto');
          refreshCount.value++;
          nextRefreshIn.value = Math.max(2, refreshSec.value);
        }, intervalMs);
        countdownTimer = setInterval(()=>{
          if(nextRefreshIn.value>0) nextRefreshIn.value--; // floor at 0
        },1000);
      } else {
    ...
    function persist(){
      try{
        const ts = Date.now();
        const st={wm:windowMode.value,w:windowVal.value,s:sinceStr.value,u:untilStr.value,unit:unit.value,lim:limit.value,ord:order.value,fut:allowFuture.value,ar:autoRefresh.value,rs:refreshSec.value,lastUpdated:ts};
        lastAppliedTs = ts;
        localStorage.setItem(STATE_KEY, JSON.stringify(st));
      }catch{}
    }
    function applyState(st){
      if(st.lastUpdated && st.lastUpdated <= lastAppliedTs) return; // ignore older
    ...
    function loadPersisted(){
      try{ const raw=localStorage.getItem(STATE_KEY); if(!raw) return; const st=JSON.parse(raw);
        if(typeof st.wm==='boolean') windowMode.value=st.wm;
        if(st.w) windowVal.value=st.w;
        if(st.s) sinceStr.value=st.s;
        if(st.u) untilStr.value=st.u;
        if(st.unit) unit.value=st.unit;
        if(st.lim) limit.value=Number(st.lim)||limit.value;
        if(st.ord) order.value=st.ord;
        if(typeof st.fut==='boolean') allowFuture.value=st.fut;
    ...
    watch([windowMode, windowVal, sinceStr, untilStr, unit, limit, order, allowFuture], persist);
  watch(autoRefresh, (val)=>{ scheduleAuto(); persist(); if(val){ emitFetch(true,'auto-init'); refreshCount.value++; }});
    ...
    function storageListener(e){
      if(e.key===STATE_KEY && e.newValue){
        try { const st = JSON.parse(e.newValue); applyState(st); } catch {}
      }
    }
    onMounted(()=>{ 
      loadPersisted(); 
  // previous draggable position removed
      // Always perform one initial fetch so persisted state (including simulated time) applies even without auto refresh.
      if(autoRefresh.value){
        scheduleAuto();
        emitFetch(true,'auto-init');
        refreshCount.value++;
      } else {
        emitFetch(true);
      }
      nextTick(updateBottomReserve);
      window.addEventListener('resize', updateBottomReserve);
      window.addEventListener('storage', storageListener);
      setTimeout(()=>{ introVisible.value = false; }, 2000);
    });
  ...
  onBeforeUnmount(()=>{ if(refreshTimer) clearInterval(refreshTimer); if(countdownTimer) clearInterval(countdownTimer); document.documentElement.style.removeProperty('--panel-bottom-pad'); window.removeEventListener('resize', updateBottomReserve); window.removeEventListener('storage', storageListener); });
    return { open, introVisible, toggle, rootEl, windowMode, windowVal, enableWindowMode, enableRangeMode, toggleMode, sinceStr, untilStr, unit, limit, order, allowFuture, autoRefresh, refreshSec, nextRefreshIn, refreshCount, simEnabled, simRunning, accel, simStartInput, nowIso, simNow, jumpValue, toggleSimRun, resetSim, applyJump, syncRealNow, emitFetch, enterFetch };
  }
};
</script>
<style scoped>
.dash-data-controls{position:fixed;left:14px;bottom:14px;width:340px;background:#ffffff;border:1px solid #14b8a6;border-radius:14px;box-shadow:0 4px 10px rgba(0,0,0,0.08),0 8px 24px rgba(0,128,128,0.18);font-size:12px;z-index:10;backdrop-filter:blur(2px);opacity:1;transition:opacity .25s ease, box-shadow .25s ease;}
...
.ddc-body{display:flex;flex-direction:column;gap:10px;padding:10px 12px;max-height:440px;overflow:auto;}
...
.section.row .btn.mode:first-child{margin-right:4px;}
.section.row .btn.mode:last-child{margin-left:4px;}
...
.refresh-input{text-align:center;}
