    ...
    <div class="card-content metrics-grid">
      ...
      <div class="grid-item gauge-item">
        ...
        >
          ...
          />
          <text x="90" y="68" text-anchor="middle" font-size="20" fill="#374151">
            {{ riskScore }}
          </text>
        </svg>
        <div class="legend">
...
export default {
  ...
  computed: {
    ...
    alertsHeight() {
      return Math.min(this.alertsCount * 12, 60);
    },
    sparklinePoints() {
      const vol = this.merchant?.transactions?.monthly?.volume ?? 0;
      const points = [vol * 0.8, vol * 0.9, vol, vol * 1.05, vol * 0.95, vol * 1.08];
      const w = 180, h = 60;
...
/* Specific Styles for Risk Metrics Card */
.metrics-grid {
  display: grid;
